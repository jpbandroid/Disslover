...
namespace Disslover
{
    public class Dissolver
    {
        ...
        public static void Main() {
            ...
            uint[] rndclr = { 0xFF0000, 0xFF000C, 0x00F33, 0xFFF700, 0x00FFEF};
            POINT[] lppoint = new POINT[3];
            while (true)
            {
                r = new Random();
                IntPtr hdc = GetDC(IntPtr.Zero);
                IntPtr whdc = CreateCompatibleDC(hdc);
                IntPtr hbit = CreateCompatibleBitmap(hdc, x, y);
                IntPtr holdbit = SelectObject(whdc, hbit);
                byte[] bits = {0xff, 0xff, 0xc3, 0xc3, 0xc3, 0xff, 0xff};
                IntPtr bitmap = CreateBitmap(8, 8, 1, 1, bits);
                IntPtr brush = CreateSolidBrush(rndclr[3]);
                int rand = r.Next(y);
